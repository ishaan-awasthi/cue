...
const METRIC_CONFIG: Array<{
  ...
  { key: "avg_wpm",           label: "Speaking Pace (WPM)",    color: "#6366f1", unit: " wpm",   yMin: 0 },
  { key: "total_fillers",     label: "Filler Words per Session", color: "#f59e0b", unit: "",       yMin: 0 },
  { key: "avg_pitch_variance",label: "Pitch Variance",          color: "#8b5cf6", unit: "",       yMin: 0 },
  { key: "avg_volume_rms",    label: "Volume (RMS)",            color: "#10b981", unit: "",       yMin: 0 },
  { key: "overall_score",     label: "Overall Score",           color: "#3b82f6", unit: "",       yMin: 0, yMax: 100 },
];
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-400 mb-1">
        ...
        <span>/</span>
        <span className="text-gray-700">Skills Progress</span>
      </div>
      <h1 className="text-2xl font-bold text-gray-900 mb-2">Skills Progress</h1>
      ...
      {allMetrics.length === 0 ? (
        ...
        <div className="space-y-10">
          {METRIC_CONFIG.map(({ key, label, color, unit, yMin, yMax }) => {
            ...
            return (
              <section key={key} className="rounded-xl border border-gray-200 bg-white p-5">
                <MetricsChart
                  data={series}
                  label={label}
                  color={color}
